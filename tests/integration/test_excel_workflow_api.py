...
class FlakyWriteExcelMcpClient(FakeExcelMcpClient):
    def __init__(self, fail_count: int) -> None:
        super().__init__()
        self._remaining_failures = fail_count

    def call_tool(self, tool_ref: str, input: dict[str, Any]) -> dict[str, Any]:
        if tool_ref == "excel.write_cells" and self._remaining_failures > 0:
            self._remaining_failures -= 1
            raise RuntimeError("forced failure: excel.write_cells")
        return super().call_tool(tool_ref, input)
...
def test_excel_workflow_end_to_end_with_approval_resume() -> None:
    ...
    try:
        ...
        assert paused["pending_approval"]["stage"] == "PLAN"

        resumed = paused
        for _ in range(30):
            if resumed["current_step_id"] is None and resumed["approval_status"] == "APPROVED":
                break
            resumed = run_api.respond_to_approval("run-1", approve=True)
    ...
    try:
        ...
        for _ in range(30):
            ...
            if (
                ...
                break
            run = run_api.respond_to_approval("run-feedback", approve=True)

        assert source_file.read_text(encoding="utf-8") == "mutated-by-create-sheet"
        updated = run_api.respond_to_approval(
            run_id="run-feedback",
...
def test_excel_workflow_supports_multiple_repair_rounds() -> None:
    base = Path(".tmp-tests") / uuid4().hex
    base.mkdir(parents=True, exist_ok=False)
    try:
        source_file = base / "sales.xlsx"
        ...
        policy_engine = DefaultPolicyEngine()
        snapshot_manager = FilesystemSnapshotManager(base / "snapshots", workspace_root=base)
        mcp_client = FlakyWriteExcelMcpClient(fail_count=2)
        ...
        for _ in range(80):
            if run["current_step_id"] is None and run["approval_status"] == "APPROVED":
                break
            run = run_api.respond_to_approval("run-multi-repair", approve=True)

        assert run["approval_status"] == "APPROVED"
        assert run["workflow_version"] == 3
        assert run["step_plan_version"] == 3
        assert run["metadata"]["repair_attempts"] == 2
        assert output_file.is_file()
    finally:
        shutil.rmtree(base, ignore_errors=True)
