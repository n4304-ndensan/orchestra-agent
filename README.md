...
- full AI mode では step plan を抽象 step に保ち、具体 MCP tool 選択は runtime に遅延
- 各 step は `finish.result` を必須の handoff として次 step に要約結果を渡す
...
`planner_mode = full` では、step plan は具体 MCP tool を直接並べるのではなく、主に `orchestra.llm_execute` / `orchestra.ai_review` の抽象 step を生成します。

- 各 step では LLM が 1 turn につき 1 action だけ返す
- action は `call_mcp_tool` / `request_file_attachments` / `write_file` / `finish`
- `finish.result` は必須で、その step の正式な要約結果として次 step に渡される
- action schema は内部 dataclass parser で検証し、将来の extension field を保持できる

詳細: [docs/llm-step-runtime.md](docs/llm-step-runtime.md)
...
- `llm.provider = openai | google` と `planner_mode = full`
  - AI が tool catalog を見て抽象 plan と step 実行を制御する汎用モード
...
- [docs/llm-step-runtime.md](docs/llm-step-runtime.md)
- [docs/workspace-artifacts.md](docs/workspace-artifacts.md)

## Repository Guide

- プロダクト状態: [docs/current-status.md](docs/current-status.md)
- step runtime protocol: [docs/llm-step-runtime.md](docs/llm-step-runtime.md)
