...
def describe_mcp_tools(mcp_client: IMcpClient | Any) -> list[dict[str, Any]]:
    describe_tools = getattr(mcp_client, "describe_tools", None)
    if callable(describe_tools):
        raw_tools = describe_tools()
        described_tools = normalize_mcp_tool_catalog(raw_tools)
        if described_tools:
            return described_tools

    return normalize_mcp_tool_catalog(mcp_client.list_tools())
